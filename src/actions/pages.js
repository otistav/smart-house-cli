...
const dispatchPages = () => {
  ...
  return (dispatch) => {
    return axios.get("http://localhost:3002/pages").then((res) => {
...
const getIcons = () => {
  ...
  return (dispatch) => {
    return axios.get("http://localhost:3002/icons").then((res) => {
...
export
const getCurrentPageControl = (id) => {

  return (dispatch) => {
    // dispatch({type: constants.CURRENT_PAGE_CONTROL_IS_FETCHING});
    return axios.get("http://localhost:3002/getPageControls?page_id=" + id).then((res) => {
